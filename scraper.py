...
#This is a random number assigned to each conference, not using it but could use it as a unique marker instead of the name
    ...
    if datebegin.find("End") != -1:
        datebegin= datebegin[0:datebegin.find("End")]
    print datebegin      
    datebstr = str(parser.parse(datebegin, fuzzy=True).date())
    
    dateend = date_css.cssselect('span.end_txt')[0].text_content()
    dateestr = str(parser.parse(dateend, fuzzy=True).date())
...
#Not ideal, using regular expression to find the link in the button
    ...
    scraperwiki.sqlite.save(unique_keys=['name'], data={'name':confname,'url':url, 'location':venue1+', '+venue2+', '+venue3, 'datebegin':datebstr, 'dateend':dateestr})           
